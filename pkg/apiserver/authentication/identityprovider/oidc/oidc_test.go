	...
	Context("OIDC", func() {
		...
		It("should configure successfully", func() {
			config := oauth.DynamicOptions{
				...
				"redirectURL":        "https://ks-console.aiscope-system.svc/oauth/redirect/oidc",
		...
		It("should login successfully", func() {
			url, _ := url.Parse("https://ks-console.aiscope-system.svc/oauth/redirect/oidc?code=00000")
			...
			Expect(identity.GetEmail()).Should(Equal("test@aiscope.io"))
		})
	})
})
